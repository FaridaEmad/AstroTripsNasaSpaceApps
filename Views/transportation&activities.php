<?php
include_once 'nav.php';
?>

<div class="top text-center pt-5 pb-5">
    <div class="container text-light">
      <h1 class="text-light">Space Activities</h1>
    </div>
  </div>
<div class="activities text-center">
  <div class="container mt-3">
    <div class="row d-flex justify-content-between py-4">
      <!--div class="col-md-6 col-lg-4"></div-->
      <div class="col-4  image-filter">
     <div class="imge_filter ">
    <img src="act1.jpg"  class="img-fluid rounded-start" alt="..."  >
  
    <h2 class="card_title text-light mb-4">Skating on Saturn</h2>
    
    <button type="button" class="btn btn-primary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="bottom" 
    data-bs-content="During the Winter seasons,you can have ablast at one of saturns famous ice skating rinks!">
      Explore Details
    </button>
    </div>
  </div>
 

  <div class="col-4 image-filter">
    <img src="../Media/IMG-20231006-WA0012.jpg"  class="img-fluid rounded-start" alt="..." >
  
    <h2 class="card_title text-light mb-4">Olympus Mons</h2>
    
    <button type="button" class="btn btn-primary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="bottom" 
    data-bs-content="It is the largest volcano in the solar system,the heights mountain and volcano in the solar system is on the planet mars">
      Explore Details
    </button>
  </div>
</div>
  <div class="row d-flex justify-content-between py-4">

  <div class="col-4 image-filter mt-5">
    <img src="../Media/IMG-20231006-WA009.jpg"  class="img-fluid rounded-start" alt="...">
  
    <h2 class="card_title text-light mb-4">Space Safari </h2>
    
    <button type="button" class="btn btn-primary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="bottom" 
    data-bs-content="During your trip you can see different animals on space which are completely different from which on the earth">
      Explore Details
    </button>
   
  </div>
  <div class="col-4 image-filter mt-5">
    <img src="act4.jpg"  class="img-fluid rounded-start" alt="..." >
  
    <h2 class="card_title text-light mb-4">Space Retreat</h2>
    
    <button type="button" class="btn btn-primary " data-bs-container="body" data-bs-toggle="popover" data-bs-placement="bottom" 
    data-bs-content="you can live in retreat like acamp and enjoying the great vibes on the space ">
      Explore Details
    </button>
   
  </div>



  
        </div>
    </div>
    </div>
</div>

<div class="top text-center pt-5 pb-5">
  <div class="container">
    <h1 class="text-light">Space Transportations</h1>
  </div>
</div>
<div class="activities text-center ">
<div class="container mt-3 ">
  <div class="row d-flex justify-content-between py-4">
    <!--div class="col-md-6 col-lg-4"></div-->
    <div class="col-4">
   <div class="image_filter ">
  <img src="ship1.jpg"  class="img-fluid rounded-start" alt="..." >

  <h2 class="card_title text-light mb-4">Apollo</h2>
  
  <button type="button" class="btn btn-primary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="bottom" 
  data-bs-content="During your trip you can take aspace ship to make atour and you can save this memories by taking some photos">
    Explore Details
  </button>
</div>
</div>


<div class="col-4">
  <img src="ship2.jpg"  class="img-fluid rounded-start" alt="..." >

  <h2 class="card_title text-light mb-4">Orion</h2>
  
  <button type="button" class="btn btn-primary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="bottom" 
  data-bs-content="you can drive aship on the space ,isn't it agreat advanture!">
    Explore Details
  </button>
 
</div>
</div>
<div class="row d-flex justify-content-between py-4">
<div class="col-4 mt-5">
  <img src="ship3.jpg"  class="img-fluid rounded-start" alt="..." >

  <h2 class="card_title text-light mb-4">Hermes </h2>
  
  <button type="button" class="btn btn-primary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="bottom" 
  data-bs-content="every ship has it's size,this ship is one of the most biggest ships on aspace and it can take many travellers on it ">
    Explore Details
  </button>
</div>
<div class="col-4 mt-5 ">
  <img src="ship4.jpg"  class="img-fluid rounded-start" alt="..." >

  <h2 class="card_title text-light mb-4">Восток</h2>
  
  <button type="button" class="btn btn-primary" data-bs-container="body" data-bs-toggle="popover" data-bs-placement="bottom" 
  data-bs-content="it is one of the strongest ships,it can travel for very long spaces ">
    Explore Details
  </button>
 
</div>
</div>
</div>
</div>

<?php
include_once 'footer.php'
?>